...
from src.entity.artifacts_entity import DataTransformationArtifact, ModelTrainerArtifact, ClassificationMetricArtifact
...
class SpamhamDetectionModel:
    ...
    def predict(self, X: DataFrame) -> DataFrame:
        spamlogging.info("Entered predict method of srcTruckModel class")

        try:
            spamlogging.info("Using the trained model to get predictions")

            transformed_feature = self.preprocessing_object.transform(X)

            spamlogging.info("Used the trained model to get predictions")
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        spamlogging.info("Entered initiate_model_trainer method of ModelTrainer class")
        ...
        try:
            ...
            if best_model_detail.best_score < self.model_trainer_config.expected_accuracy:
                            spamlogging.info("No best model found with score more than base score")
            ...
            spamlogging.info("Spam Ham detection Model is created and saved.")
            ...
            spamlogging.info(f"Spam Ham detection Model is saved successfully at: {trained_model_path}")
            ...
            spamlogging.info("Model training completed successfully")
            spamlogging.info(f"Model trainer artifact: {model_trainer_artifact}")
